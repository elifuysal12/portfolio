#!/usr/bin/env python3
"""Build the Jotform chatbot dashboard room into ../demo/jotform/.

    python3 src/build_demo_jotform.py

A monitor on a dark desk with Elif's Figma prototype running inside it, under
the site's own nav — the desktop counterpart of build_demo_alti.py's phone.

Two things were tried here first and are worth not repeating:

  1. A bare embed, edge to edge. It reads as a Figma player someone dropped in
     a page, and its loading state is a white rectangle the size of the room.
  2. A coded rebuild of the dashboard. It ran, but the icons and a hundred
     small decisions were mine rather than hers — a portfolio piece has to be
     the designer's own artefact, not a good-faith copy of it. Deleted; the
     source is kept outside the repo at ~/Projects/jotform-dashboard.

So the screen holds the prototype, and the frame around it does the work the
embed could not: it says "desktop product" before anyone reads a word, and it
gives the player a dark room to load into.

The prototype must stay shared publicly in Figma — Share → *Anyone with the
link* → can view. That is Elif's own account setting; the room carries a
visible "open in Figma" escape hatch in case it ever lapses.
"""
...
# straight off the prototype's share link
FILE  = '2x6KiLkwK5G2oheLZrzSaS'
NAME  = 'Jotform'
NODE  = '1384-158976'
PAGE_ID = '930:24958'
START = '1384:146915'
# `scale-down-width`, never `contain`: with the screen cut to the frame's own
# 1440x900 the width decides and the height lands exactly, so there is no
# letterbox and no scroll. `contain` would fit the frame at about half size
# inside a black surround — see the same note in build_demo_alti.py.
QS = (f"node-id={NODE}&page-id={quote(PAGE_ID,safe='')}"
      f"&starting-point-node-id={quote(START,safe='')}"
      "&scaling=scale-down-width&content-scaling=fixed")
EMBED = f"https://embed.figma.com/proto/{FILE}/{NAME}?{QS}&hide-ui=1&embed-host=elifbeyzauysal"
PROTO = f"https://www.figma.com/proto/{FILE}/{NAME}?{QS}"

TITLE = 'Chatbot Dashboard — interactive prototype · Elif Beyza Uysal'
DESC = ('An analytics dashboard for Jotform AI Chatbot, embedded in WordPress — '
        'the clickable prototype, by Elif Beyza Uysal.')
...
# The room wears the site's nav, so it needs the site's two families. Linked
# rather than inlined, like the other rooms: they share one cached copy, and a
# shell about to pull down a whole prototype should not carry 150 KB of base64
# in front of it. Written here too, so no build depends on another having run.
...
FONTS = ''.join((
    ...
    face('Bricolage Grotesque', 'br-latin.woff2', LATIN, '800'),
    face('Bricolage Grotesque', 'br-latinext.woff2', LATIN_EXT, '800'),
))
...
/* THE SITE'S OWN NAV, not a bar invented for this room — same shape, same
   type, same order, carried into the dark the way body.case-open carries it
   on a case study. The section links are absolute because this is a separate
   address. */
nav{{
...
.lang button{{font-family:inherit;font-size:12.5px;border:0;cursor:pointer;padding:5px 9px;
...
/* ---- the desk ----
   Three columns, the same shape as the phone room: the words on the left, the
   controls on the right, and the middle left to the object. Stacking them
   under the monitor cost it height on every screen and gave the page a column
   of equal-weight things instead of one machine to look at. */
main{{flex:1 1 auto;min-height:0;display:grid;
  grid-template-columns:1fr auto 1fr;align-items:center;
  gap:clamp(18px,3vw,52px);padding:84px 22px 16px}}
/* narrower than its column on purpose: with justify-self:end the block
   then sits beside the monitor instead of against the window edge */
.side{{max-width:29ch}}
.side.left{{justify-self:end}}
.side.right{{justify-self:start}}
.room h1{{margin:0;font-family:'Bricolage Grotesque',system-ui,sans-serif;font-weight:800;
...
.hint{{margin:24px 0 0;font-size:15px;line-height:1.72;color:#A9ACB8}}
...
.mon{{position:relative;padding:14px 14px 30px;border-radius:20px;
...
.mon:after{{content:'';position:absolute;left:50%;bottom:13px;width:5px;height:5px;
  margin-left:-2.5px;border-radius:50%;background:#5EE38A;
  box-shadow:0 0 8px rgba(94,227,138,.75)}}
/* the screen is cut to the frame's own 1440x900 and sized against whatever the
   nav, the stand and the two side columns leave — in CSS, because the middle
   column of a `1fr auto 1fr` grid is measured *from* this box, so asking JS
   for the space first would be asking it to measure itself */
.screen{{position:relative;border-radius:7px;overflow:hidden;background:#F0F0F1;
  box-shadow:0 0 0 1px rgba(0,0,0,.55) inset;
  aspect-ratio:1440/900;
  height:min(900px, calc(100dvh - 236px), calc((100vw - 700px) * .625));}}
...
.wait .bar i{{display:block;width:40%;height:100%;border-radius:3px;background:var(--ramp);
...
.foot{{width:280px;height:16px;border-radius:0 0 14px 14px;
  background:linear-gradient(#2A2D35,#15171C);box-shadow:0 22px 34px -18px rgba(0,0,0,.9)}}

/* THE ACTIONS, in the site's own two weights — `.demo-btn` from a case study
   and `.go`/`.quiet` from the phone room, to the pixel: same pill, same
   14.5px, same 14/22, same ramp, same spring lift. The secondary is NOT an
   outlined copy of the primary; an outline-only control is on the list of
   tells to keep off this site. The filled one is *Start over*, because in a
   room built around a prototype the thing to press is the prototype again. */
...
@media (max-width:1100px){{
  body{{overflow:auto}}
  main{{grid-template-columns:1fr;justify-items:center;text-align:center;
    padding:74px 12px 10px;gap:22px}}
  .side{{max-width:46ch;justify-self:center}}
  .acts{{align-items:center}}
  .screen{{height:min(900px, calc((100vw - 48px) * .625))}}
  .mon{{padding:9px 9px 20px;border-radius:14px}}
}}
@media (max-width:640px){{
  .mon{{padding:6px 6px 6px;border-radius:11px}}
  .mon:after,.neck,.foot{{display:none}}
  nav{{top:10px;padding-left:14px}} nav a{{padding:8px 9px}}
}}
</style>
{BEACON}
</head>
...
<main>
  <div class="side left">
    <div class="room">
      ...
      <span data-i18n="sub">Jotform &middot; inside WordPress</span>
    </div>
    <p class="hint" data-i18n-html="hint">This is the prototype itself, not a picture of it.
      <b>Click the sidebar, the tabs and the cards.</b> Overview, User Rates and
      Conversation Rates are all wired.</p>
  </div>

  <div class="rig">
    <div class="mon">
      <div class="screen" id="screen">
        <iframe id="proto" src="{EMBED}" title="Chatbot Dashboard — prototype"
...
<p class="foot-note" data-i18n-html="foot"></p>

<script>
(function(){{
  var proto=document.getElementById('proto'), wait=document.getElementById('wait');
  proto.addEventListener('load', function(){{
    /* the frame fires load before the player has painted, so give it a beat
       rather than swapping a dark card for a white one */
    setTimeout(function(){{ proto.classList.add('ready'); wait.classList.add('gone'); }}, 900);
  }});
...
/* The nav carries the site's language switch, so it has to actually switch.
   The room is a dozen words, so this is all of it — no persistence, which is
   what the site does too (it opens in English every time). */
var CREDIT = '<a href="https://elifbeyzauysal.com/">Elif Beyza Uysal</a>';
var T = {{
...
# the coded rebuild that used to live here is gone: it was not her drawing
old = os.path.join(OUT, 'app')
if os.path.isdir(old):
    shutil.rmtree(old)
    print('removed demo/jotform/app (the coded rebuild)')

print(f'demo/jotform/index.html  {len(PAGE.encode())/1024:.1f} KB  → {FILE} {NODE}')
